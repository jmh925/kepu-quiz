...
| TC-08 | 题目字段完整性 | 检查首题字段 | 含题干/选项/答案下标/解析/知识点 | 字段=8 选项数=4 answer=2 | 通过 |
...
| TC-21 | 知识库文档上传 | POST /api/v1/knowledge/documents (txt) | code=0 且返回分块数 | 文档ID=4f15673f9ae7 分块=1 | 通过 |
...
| TC-31 | 管理端运行看板 | GET /api/v1/admin/dashboard | code=0 且含用户/闯关/错题统计 | 用户数=13 闯关数=53 平均正确率=72.9 | 通过 |
| TC-32 | 管理端新增题目 | POST /api/v1/admin/questions | code=0 且返回新题 ID | 新题ID=3 | 通过 |
| TC-33 | 管理端题库查询 | GET /api/v1/admin/questions?keyword=月球 | code=0 且命中 1 条 | 命中=1 | 通过 |
...
| TC-36 | 资源池题目参与出题 | 出题时优先命中管理端题库 | 题目中包含新增题 | 命中=True | 通过 |
| TC-37 | 管理端删除题目 | DELETE /api/v1/admin/questions/{id} | code=0 | code=0 | 通过 |
| TC-38 | 管理端用户列表 | GET /api/v1/admin/users | code=0 且含闯关数统计 | 用户数=13 | 通过 |
| TC-39 | 管理端闯关记录 | GET /api/v1/admin/sessions | code=0 | 记录数=5 总数=54 | 通过 |
| TC-40 | 管理端操作日志留痕 | GET /api/v1/admin/logs | code=0 且包含题库操作记录 | 日志条数=12 | 通过 |

## 表 6-2 接口响应时间测试结果

| 接口 | 功能 | 重复次数 | 平均响应时间(ms) | 最小(ms) | 最大(ms) |
| --- | --- | --- | --- | --- | --- |
| GET /api/v1/grades | 学段选项查询 | 3 | 17 | 15 | 22 |
| POST /api/v1/user/login | 用户登录 | 3 | 11 | 11 | 12 |
| POST /api/v1/quiz/generate | 出题（题库降级路径） | 3 | 25 | 13 | 31 |
| POST /api/v1/quiz/submit | 判题与结算 | 3 | 30 | 24 | 43 |
| POST /api/v1/report/generate | 复盘报告（规则降级路径） | 3 | 20 | 14 | 30 |
| GET /api/v1/wrong/questions | 错题本查询 | 3 | 4 | 4 | 5 |
| POST /api/v1/wrong/practice | 只练错题组卷 | 3 | 28 | 23 | 31 |
