...
| TC-21 | 知识库文档上传 | POST /api/v1/knowledge/documents (txt) | code=0 且返回分块数 | 文档ID=02ad9fd447eb 分块=1 | 通过 |
...
| TC-31 | 管理端运行看板 | GET /api/v1/admin/dashboard | code=0 且含用户/闯关/错题统计 | 用户数=14 闯关数=11 平均正确率=34.4 | 通过 |
| TC-32 | 管理端新增题目 | POST /api/v1/admin/questions | code=0 且返回新题 ID | 新题ID=1 | 通过 |
...
| TC-38 | 管理端用户列表 | GET /api/v1/admin/users | code=0 且含闯关数统计 | 用户数=14 | 通过 |
| TC-39 | 管理端闯关记录 | GET /api/v1/admin/sessions | code=0 | 记录数=5 总数=12 | 通过 |
| TC-40 | 管理端操作日志留痕 | GET /api/v1/admin/logs | code=0 且包含题库操作记录 | 日志条数=8 | 通过 |

## 表 6-2 接口响应时间测试结果

| 接口 | 功能 | 重复次数 | 平均响应时间(ms) | 最小(ms) | 最大(ms) |
| --- | --- | --- | --- | --- | --- |
| GET /api/v1/grades | 学段选项查询 | 3 | 11 | 2 | 16 |
| POST /api/v1/user/login | 用户登录 | 3 | 24 | 10 | 32 |
| POST /api/v1/quiz/generate | 出题（题库降级路径） | 3 | 26 | 10 | 36 |
| POST /api/v1/quiz/submit | 判题与结算 | 3 | 41 | 40 | 43 |
| POST /api/v1/report/generate | 复盘报告（规则降级路径） | 3 | 24 | 22 | 26 |
| GET /api/v1/wrong/questions | 错题本查询 | 3 | 18 | 16 | 23 |
| POST /api/v1/wrong/practice | 只练错题组卷 | 3 | 28 | 22 | 32 |
