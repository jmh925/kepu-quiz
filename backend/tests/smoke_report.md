...
| TC-21 | 知识库文档上传 | POST /api/v1/knowledge/documents (txt) | code=0 且返回分块数 | 文档ID=08df52161006 分块=1 | 通过 |
...
| TC-31 | 管理端运行看板 | GET /api/v1/admin/dashboard | code=0 且含用户/闯关/错题统计 | 用户数=129 闯关数=276 平均正确率=59.6 | 通过 |
| TC-32 | 管理端新增题目 | POST /api/v1/admin/questions | code=0 且返回新题 ID | 新题ID=165 | 通过 |
...
| TC-38 | 管理端用户列表 | GET /api/v1/admin/users | code=0 且含闯关数统计 | 用户数=129 | 通过 |
| TC-39 | 管理端闯关记录 | GET /api/v1/admin/sessions | code=0 | 记录数=5 总数=277 | 通过 |
...
| --- | --- | --- | --- | --- | --- |
| GET /api/v1/grades | 学段选项查询 | 3 | 16 | 15 | 16 |
| POST /api/v1/user/login | 用户登录 | 3 | 14 | 13 | 15 |
| POST /api/v1/quiz/generate | 出题（题库降级路径） | 3 | 31 | 30 | 33 |
| POST /api/v1/quiz/submit | 判题与结算 | 3 | 35 | 21 | 42 |
| POST /api/v1/report/generate | 复盘报告（规则降级路径） | 3 | 23 | 12 | 34 |
| GET /api/v1/wrong/questions | 错题本查询 | 3 | 18 | 4 | 25 |
| POST /api/v1/wrong/practice | 只练错题组卷 | 3 | 18 | 13 | 27 |
