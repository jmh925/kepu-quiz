...
| TC-21 | 知识库文档上传 | POST /api/v1/knowledge/documents (txt) | code=0 且返回分块数 | 文档ID=75dfe00c9ff2 分块=1 | 通过 |
...
| TC-31 | 管理端运行看板 | GET /api/v1/admin/dashboard | code=0 且含用户/闯关/错题统计 | 用户数=76 闯关数=175 平均正确率=57.0 | 通过 |
| TC-32 | 管理端新增题目 | POST /api/v1/admin/questions | code=0 且返回新题 ID | 新题ID=9 | 通过 |
...
| TC-38 | 管理端用户列表 | GET /api/v1/admin/users | code=0 且含闯关数统计 | 用户数=76 | 通过 |
| TC-39 | 管理端闯关记录 | GET /api/v1/admin/sessions | code=0 | 记录数=5 总数=176 | 通过 |
...
| POST /api/v1/user/login | 用户登录 | 3 | 23 | 12 | 37 |
| POST /api/v1/quiz/generate | 出题（题库降级路径） | 3 | 23 | 13 | 33 |
| POST /api/v1/quiz/submit | 判题与结算 | 3 | 40 | 37 | 45 |
| POST /api/v1/report/generate | 复盘报告（规则降级路径） | 3 | 30 | 17 | 39 |
| GET /api/v1/wrong/questions | 错题本查询 | 3 | 20 | 18 | 21 |
| POST /api/v1/wrong/practice | 只练错题组卷 | 3 | 35 | 31 | 38 |
