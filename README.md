...
一个面向中小学生的闯关答题系统。孩子输入想学的主题（例如「太阳系」「彩虹是怎么来的」），
系统现场出题、逐题讲解、给出复盘报告，并把答错的题沉淀成错题本；
也能直接选**语文 / 数学 / 英语**三大基础课程刷题，或者**随机匹配一位同学 PK** 看谁答得准；
...
| PK 对战（学生端） | 随机匹配开一局 | — | — | 战绩、连胜、逐题对照 |
| 题库资源池（管理端） | 新增题目 | 删单条 | 改题干、选项、答案、难度、启用状态 | 分页 + 关键词搜索 |
| 用户（管理端） | — | — | 启用 / 停用 | 分页 + 关键词搜索 |
| 运行数据（管理端） | — | — | — | 看板、趋势、闯关记录、操作日志 |

**题库**：内置 **225 道**分级题目，无 API Key 时全靠它出题。

| 类别 | 主题 | 题量 |
| --- | --- | --- |
| 科普主题 | 天文 / 地理 / 生物 / 物理 / 化学 / 科技 | 每主题 25 题 |
| 基础课程 | 语文 / 数学 / 英语 | 每主题 25 题 |

每个主题内部都按学段分开（小学低年级 / 小学高年级 / 初中各 8~9 题），
**低年级与初中不共用题干**，避免给小一生出初中的题。
数学题的答案由 `tools/gen_math_bank.py` 计算得出（不是手写），
`tools/math_specs.json` 记录了每道题对应的算式，可独立复核。

**趣味化设计**（面向小朋友，而不是给成人看的表单）：

- 每题**倒计时**给节奏感，但到点**不判负、不扣分**，只把小科换成鼓励态
- **连对**会累计并喝彩（「三连对，稳住！」），答对的音高随连对次数升高
- **8 个段位的成长阶梯**：从 🌱科学小新芽 一路爬到 🏆传奇科学家（1250 经验），
  段位表有独立页面，明确标出「你在这里」和「离最高段位还差多少」；
  每档有等级区间、配色与标语，顶栏胶囊可点击直达
- **PK 对战**：随机匹配一位对手，同一份卷子比谁答得准。对手优先取
  **真实同学在同一份卷上的战绩**（带昵称与他的真实段位），抽不到才用机器人兜底——
  所以一个人在场也能开局，赢了额外加经验并累计连胜
...
| 链路 | 首选方案 | 降级方案 |
| --- | --- | --- |
| 出题 | 调用 DeepSeek 生成题目 | 内置 225 题分级题库（科普 6 主题 + 语文/数学/英语）/ 管理端题库资源池（按主题匹配 + 随机抽样） |
...
**学生端（24 个）**：`GET /health`、`GET /grades`、`GET /themes`、`GET /levels`、
...
`POST /pk/start`、`POST /pk/finish`、`GET /pk/stats`、
...
`4004` 对战不存在或已结算 · `4010` 学生端未登录 · `4011` 管理端未认证 · `5000` 服务器内部错误。
