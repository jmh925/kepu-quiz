...
def main():
    ...
    print("分支：%s" % branch)
    print("本地提交：%s" % head[:10])
    print("文件数：%d" % len(files))
    print("待上传提交数：%d" % max(1, len(chunks)))
    print("提交信息首行：%s" % subject.split("\n")[0][:70])
    if args.dry_run:
        for f in files[:15]:
            print("   " + f)
        return 0
    ...
    try:
        ...
        remote_head = None
    print("远程当前：%s" % (remote_head[:10] if remote_head else "（空）"))
    if remote_head == head:
        print("已经是同一个提交，无需上传")
        return 0
    ...
    print("已更新 %s 分支 → %s" % (branch, commit["sha"][:10]))
    print("仓库地址：https://github.com/%s" % OWNER_REPO)
    return 0
